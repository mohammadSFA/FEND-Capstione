async function apiGet(location, departure, returnValue) {
    const req = await fetch(`localhost:8001/api?location=${location}&departure=${departure}&returnValue=${returnValue}`)
    try {
...
function appRunner(event) {
    ...
    apiGet(location, departure, returnValue).then((object)=>{
        const { lengthOfTrip, temperature, imgURL } = object
